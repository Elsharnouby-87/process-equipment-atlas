...
const viewCopy: Record<BurnerStudy, { eyebrow: string; title: string; body: string; watch: string[] }> = {
  external: { eyebrow: 'OPERATOR PERSPECTIVE', title: 'Underfurnace access without exposed main flame', body: 'The heater is elevated so burner external hardware can be inspected and adjusted from below. The working flame remains above the burner floor, inside the radiant firebox.', watch: ['Air-register travel and actuator condition', 'Fuel-line, valve and connection integrity', 'Pilot / ignition access', 'Clear working space around the burner body'] },
  internal: { eyebrow: 'FIREBOX PERSPECTIVE', title: 'See where the burner actually fires', body: 'The internal cutaway follows the burner from its external body through the burner floor, refractory throat and flame root into the radiant section. The flame is contained inside the heater.', watch: ['Stable flame root', 'Burner-tile condition', 'Clearance from radiant tubes', 'No abnormal flame pull, lift or impingement'] },
  exploded: { eyebrow: 'ASSEMBLY STUDY', title: 'Separate the burner hardware — not the whole heater', body: 'This study view spreads the burner assembly elements apart while the heater remains as ghosted location context. It is a training representation of assembly relationships, not an OEM maintenance drawing.', watch: ['Register and body relationship', 'Fuel-gun path toward the flame root', 'Tile / throat position above the floor', 'Pilot and sensing hardware alongside the main burner'] },
  pilot: { eyebrow: 'IGNITION & FLAME PROVING', title: 'Pilot, ignition electrode and flame detection', body: 'The close study view highlights the pilot / ignition region. Pilot port, ignition electrode, flame rod or other flame-proving hardware depend on the burner and BMS design, so the arrangement shown here is schematic.', watch: ['Electrode and pilot alignment', 'Pilot flame location relative to the main burner', 'Flame-proving hardware condition', 'Wiring / connection condition where accessible'] },
};
    ...
    <main className="app-shell burner-page">
      ...
      <section className="burner-workspace">
        ...
        <aside className="burner-tech">
          ...
          {study === 'pilot' && <section className="burner-tech-section pilot-micro-copy"><span>DEDICATED PILOT MICRO-VIEW</span><p>The 3D view isolates a schematic pilot port, ignition electrode, ground reference, flame-proving element and pilot flame. Exact geometry and proving technology vary by OEM and BMS design.</p></section>}
